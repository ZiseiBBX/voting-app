...
function Home() {
  ...
  if (status === "loading") return <Flex width="full" justifyContent="center" direction="column"><Box>Loading</Box></Flex>
  ...
  return (
    <PageContainer>
      <Flex justifyContent="space-between" mt="5" px={["5", "20"]} alignItems="center" w="full">
        <PageHeader title="Polls" />
        {user.email !== "" && <Button size="sm" variant="solid"><Link to="/new-poll">Add New Poll</Link></Button>}
      </Flex>
      {data.length === 0 && <Flex justifyContent="center" alignItems="center" h="md">No polls so far O_O</Flex>}
      <Box w="full" px={["5", "20"]} mt="5">
        <Accordion maxHeight="500" w="full" overflow="scroll" overflowX="hidden" css={{...customScrollbar(scrollbarColor), ":focus outline": "none" }} allowToggle>
          {data.map((datum) => {
            let totalVotes = 0
            ...
            return <AccordionItem key={`${datum.title}${datum.uid}`} border="1px" rounded="md" my="2">
              <AccordionButton>
                <Box flex="1" textAlign="left">{datum.title}</Box>
                <AccordionIcon />
              </AccordionButton>
              <AccordionPanel px={["2", "10"]}>
                <Box>
                  {datum.options.map((option, optionIndex) => {
                    ...
                    return <Box px="4" py="1" my="2" key={option.title}>
                      <Flex w="full" alignItems="center">
                        <Text width="10" flex="1">{option.title}</Text>
                  ...
                  <Flex justifyContent="center" textAlign="center" px="20">
                    ...
                    {(user.email !== "" && !voted) &&
                      <Flex>
                        <Select mx="2" placeholder="Select option..." size="sm" minWidth="40" onChange={(e: BaseSyntheticEvent) => setSelectedOption(e.target.value)}>
                          { datum.options.map((option) => {
                            return <option key={option.title} value={option.title}>{option.title}</option>
                          }) }
                        </Select>
                        <Button mx="2" px="8" size="sm" variant="solid" onClick={async () => {
                          if (!selectedOption) {
